<?php 	
	...
	function obtenerNombreClienteProveedor($idRegistro,$tipoBus)
	{
		...
		switch($tipoBusqueda)
		{
			...
			break;
			case 2:
					$consulta="SELECT IF(tipoEmpresa=2,nombre,CONCAT(nombre,' ',apPaterno,' ',apMaterno))AS nombre FROM 3003_proveedores WHERE idProveedor='".$idRegistro."'";
			break;
		}
	...
	function obtenerDatosIVA($id)
	{
		global $con;

		$consulta="SELECT nombreImpuesto,valor FROM 4002_impuesto WHERE idImpuesto='".$id."'";
		$res=$con->obtenerPrimeraFila($consulta);

		$text=strtoupper($res[0]);
		$valor=$res[1];
	...
	function obtenerNombrePaciente($id)
	{
		...
		$consulta="SELECT nombre,apPaterno,apMaterno FROM 3005_pacientes WHERE idPaciente='".$id."'";
		$res=$con->obtenerPrimeraFila($consulta);

		$valor=strtoupper($res[0]." ".$res[1]." ".$res[2]);
