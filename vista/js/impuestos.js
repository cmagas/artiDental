...
function listarImpuestos() {
    table = $("#tbl_impuestos").DataTable({
        ...
        processing: true,
        ajax: {
          url: "funciones/paginaFuncionesImpuestos.php",
          type: "POST",
          data:{
              funcion: "1"
          }
...
$("#tbl_impuestos").on("click", ".btEliminar", function () {
    var data = table.row($(this).parents("tr")).data();
    ...
    Swal.fire({
      title: "¿Esta seguro de querer Eliminar el Impuesto?",
      ...
      if (result.value) {
        modificarEstatusImpuesto(data.id, "0");
      }
    });
});
...
function registrar_impuesto()
{
    ...
    obtenerDatosWeb('funciones/paginaFuncionesImpuestos.php',funcAjax, 'POST','funcion=2&cadObj='+cadObj,true) 
}
...
function modificar_impuesto()
{
    ...
    obtenerDatosWeb('funciones/paginaFuncionesImpuestos.php',funcAjax, 'POST','funcion=3&cadObj='+cadObj,true)

}

function modificarEstatusImpuesto(id, estado)
{
    var mensaje = "";
    var idImpuesto = id;
    ...
    var cadObj = '{"idImpuesto":"' + idImpuesto + '","estado":"' + estado + '"}';
    ...
    function funcAjax() {
      ...
      if (arrResp[0] == "1") {
        listarImpuestos();
        Swal.fire("Mensaje De Confirmacion","El Impuesto se " + mensaje + " con exito","success");
      } else {
        Swal.fire("Mensaje De Error","Lo sentimos, no se pudo modificar el registro","error");
      }
    }
    obtenerDatosWeb("funciones/paginaFuncionesImpuestos.php",funcAjax,"POST","funcion=4&cadObj=" + cadObj,true);
}
